...
## Wrangling - pivots, joins, mutate and summarize {.scrollable}
...
### Mutate and Summarize {.scrollable}
...
met_1 <- match_2 %>%
  group_by(EventID, trait) %>% 
  summarize(richness = length(ct_trait), 
            sum = sum(ct_trait), .groups = 'keep') %>%
  ungroup()

# This is equivalent to the following:
met_1_alt <- match_2 %>%
  ddply(c('EventID', 'trait'), plyr::summarize, 
        richness = length(ct_trait),
        sum = sum(ct_trait))
...
met_2_long <- met_2 %>%
  pivot_longer(cols = c(richness:sum, prop)) %>%
  mutate(metric = paste(trait, name, sep = '_')) 
...
## Prepare data for output {.scrollable}

### Pivot data wider {.scrollable}
...
### Export data to a file {.scrollable}
